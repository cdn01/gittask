...
if(@$rs[0]['id']>0)
{
	$hotword_sql = "select * from hotword order by postnum  asc ,id asc limit 2";
	$hotword_rs = query($hotword_sql);
	$hotword = "#".$hotword_rs[0]["hotword"]."#"."  "."#".$hotword_rs[1]["hotword"]."#";
	...
	$message = $hotword.trim($rs[0]['title']).$users;
	// $message = trim($rs[0]['title']);
